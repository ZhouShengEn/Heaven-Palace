...
DictItem.openAddDict = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/business/dict/dict_add'
    });
...
DictItem.openDictDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/business/dict/item/dict_edit/' + DictItem.seItem.id
        });
        this.layerIndex = index;
    }
};
...
DictItem.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/business/dict/item/delete", function (data) {
                Feng.success("删除成功!");
...
DictItem.envChange = function (option, value) {
    ...
    ajax.set("env", option);
    ajax.set("cacheParam", "businessDictItem");
    ajax.start();
...
DictItem.reloadLog = function () {
    ...
    ajax.set("type", "BUSINESS_DICT_ITEM");
    ajax.set("cacheParam", "businessDictItem");
    ajax.start();
...
$(function () {
    ...
    var table = new BSTable(DictItem.id, "/business/dict/item/list", defaultColunms);
