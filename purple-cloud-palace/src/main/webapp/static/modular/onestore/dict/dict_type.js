...
DictType.openAddDict = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/business/dict/type/dict_add'
    });
...
DictType.openDictDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/business/dict/type/dict_edit/' + DictType.seItem.id
        });
        this.layerIndex = index;
    }
};
...
DictType.delete = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/business/dict/type/delete", function (data) {
                Feng.success("删除成功!");
...
DictType.envChange = function (option, value) {
    var ajax = new $ax(Feng.ctxPath + "/business/env/change", function (data) {
        Feng.success("切换成功!");
    ...
    ajax.set("env", option);
    ajax.set("cacheParam", "businessDictType");
    ajax.start();
...
DictType.reloadLog = function () {
    ...
    ajax.set("type", "BUSINESS_DICT_TYPE");
    ajax.set("cacheParam", "businessDictType");
    ajax.start();
...
$(function () {
    ...
    var table = new BSTable(DictType.id, "/business/dict/type/list", defaultColunms);
