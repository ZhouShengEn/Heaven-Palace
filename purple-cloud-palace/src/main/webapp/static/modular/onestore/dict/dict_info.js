...
DictItemDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/business/dict/item/add", function (data) {
        Feng.success("添加成功!");
...
DictItemDlg.editSubmit = function () {
    ...
    var ajax = new $ax(Feng.ctxPath + "/business/dict/item/update", function (data) {
        Feng.success("修改成功!");
        window.parent.DictItem.table.refresh();
        window.parent.DictItem.reloadLog();
        DictItemDlg.close();
...
$(function () {
    ...
    var ztree = new $ZTree("dictTypeTree", "/business/dict/type/tree/");
